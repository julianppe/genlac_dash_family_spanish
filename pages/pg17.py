...
dash.register_page(__name__,
                   path='/horas-actividades-apoyo',  # represents the url text
                   name='Horas semanales dedicadas a actividades de apoyo a otros hogares',  # name of page, commonly used as name of link
                   title='Horas semanales dedicadas a actividades de apoyo a otros hogares'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/hs_ayuda_otros.csv")
...
layout = html.Div([
        ...
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page17-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page17-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page17-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page17-pais_elect', "value"),
    Output("store", "data"),
    Input('page17-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page17-line', 'figure'),
    Input('page17-pais_elect', 'value'),
    Input('page17-comparacion_por_elect', 'value'),
    [Input('page17-the_year','value')]
)
