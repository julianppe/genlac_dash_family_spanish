...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page6-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page6-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page6-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page6-pais_elect', "value"),
    Output("store", "data"),
    Input('page6-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page6-line', 'figure'),
    Input('page6-pais_elect', 'value'),
    Input('page6-comparacion_por_elect', 'value'),
    [Input('page6-the_year','value')]
)
