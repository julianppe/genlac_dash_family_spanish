...
dash.register_page(__name__,
                   path='/tasa-fecundidad',  # represents the url text
                   name='Tasa de fecundidad',  # name of page, commonly used as name of link
                   title='Tasa de fecundidad'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/fecundidad.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page20-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Mujeres', id='page20-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page20-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page20-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page20-pais_elect', "value"),
    Output("store", "data"),
    Input('page20-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page20-line', 'figure'),
    Input('page20-pais_elect', 'value'),
    Input('page20-comparacion_por_elect', 'value'),
    [Input('page20-the_year','value')]
)
