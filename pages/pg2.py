...
dash.register_page(__name__,
                   path='/diferencia-educativa',  # represents the url text
                   name='Diferencia educativa entre conyuges',  # name of page, commonly used as name of link
                   title='Diferencia educativa entre conyuges'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/diferencia_educ.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page2-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page2-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page2-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page2-the_year',
                min=2000,
                max=2020,
                value=[2000,2020],
                marks=mark_values,
...
@callback(
    Output('page2-pais_elect', "value"),
    Output("store", "data"),
    Input('page2-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page2-line', 'figure'),
    Input('page2-pais_elect', 'value'),
    Input('page2-comparacion_por_elect', 'value'),
    [Input('page2-the_year','value')]
)
