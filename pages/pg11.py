...
dash.register_page(__name__,
                   path='/horas-tareas-domesticas',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/hs_tareas_domesticas.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page11-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page11-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page11-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page11-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page11-pais_elect', "value"),
    Output("store", "data"),
    Input('page11-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page11-line', 'figure'),
    Input('page11-pais_elect', 'value'),
    Input('page11-comparacion_por_elect', 'value'),
    [Input('page11-the_year','value')]
)
