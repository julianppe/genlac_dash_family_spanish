...
dash.register_page(__name__,
                   path='/actividades-ocio',  # represents the url text
                   name='Participación en actividades de ocio',  # name of page, commonly used as name of link
                   title='Participación en actividades de ocio'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/ocio.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page18-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Ratio mujeres/hombres', id='page18-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page18-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page18-the_year',
                min=2000,
                max=2021,
                value=[2000,2021],
                marks=mark_values,
...
@callback(
    Output('page18-pais_elect', "value"),
    Output("store", "data"),
    Input('page18-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page18-line', 'figure'),
    Input('page18-pais_elect', 'value'),
    Input('page18-comparacion_por_elect', 'value'),
    [Input('page18-the_year','value')]
)
