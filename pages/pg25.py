...
dash.register_page(__name__,
                   path='/metodo-anticonceptivos-modernos',  # represents the url text
                   name='Porcentaje de mujeres que utilizan métodos anticonceptivos modernos',  # name of page, commonly used as name of link
                   title='Porcentaje de mujeres que utilizan métodos anticonceptivos modernos'  # epresents the title of browser's tab
)


# page 1 data
df = pd.read_csv("datasets/anticoncepcion_moderna.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page25-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Mujeres', id='page25-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page25-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
...
@callback(
    Output('page25-pais_elect', "value"),
    Output("store", "data"),
    Input('page25-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page25-line', 'figure'),
    Input('page25-pais_elect', 'value'),
    Input('page25-comparacion_por_elect', 'value'),
    [Input('page25-the_year','value')]
)
...
def update_graphs(pais_v, comparacion_por_v, years_chosen):
    ...
    if comparacion_por_v == 'Brecha mujeres - hombres':
        ...
        fig_line = px.bar(dff, x='ano', y='valor', color='pais', pattern_shape='desagregacion', pattern_shape_sequence=["", "x", "."], 
