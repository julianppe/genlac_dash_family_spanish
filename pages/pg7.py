...
dash.register_page(__name__,
                   path='/jefatura-hogar-auto',  # represents the url text
...
# page 1 data
df = pd.read_csv("datasets/jefa_mujer.csv")
...
layout = html.Div([
        dbc.Row([
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in df.pais.unique()], multi=True, id='page7-pais_elect')
        ], width=6),
        dbc.Col([
            dcc.Dropdown(options=[{'label': x, 'value': x} for x in list_comparacion_por_ordenada], multi=False, persistence=True, persistence_type='memory', value='Total', id='page7-comparacion_por_elect')
        ], width=6),
    ]),
        dbc.Row([
        dbc.Col([
            dcc.Graph(id='page7-line', config={'displayModeBar':False})
        ], width=12),
    ]),
        dbc.Row([
        dbc.Col([
        dcc.RangeSlider(id='page7-the_year',
                min=2000,
                max=2020,
                value=[2000,2020],
                marks=mark_values,
...
@callback(
    Output('page7-pais_elect', "value"),
    Output("store", "data"),
    Input('page7-pais_elect', "value"),
    State("store", "data"),
)
...
@callback(
    Output('page7-line', 'figure'),
    Input('page7-pais_elect', 'value'),
    Input('page7-comparacion_por_elect', 'value'),
    [Input('page7-the_year','value')]
)
